...
const UserAnalytics = ({isDashboard}:Props) => {
  ...
  useEffect(() => {
    if (data) {
      ...
      const lastTwoMonths = data.users.last12Months.slice(-2);
  ...
  // Custom Tooltip cho hiển thị chi tiết khi hover
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white dark:bg-gray-800 p-3 shadow-lg rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <p className="text-purple-600 dark:text-purple-400 font-semibold">{`Số người dùng: ${payload[0].value}`}</p>
        </div>
      );
    }
    return null;
          ...
          >
            <div className={`${isDashboard ? "p-4" : "mt-8 p-5"}`}>
              <h1 className={`${styles.title} ${isDashboard && "!text-[20px]"} !text-start flex items-center`}>
                <span className="mr-2">👥</span> Thống kê người dùng
              </h1>
              {!isDashboard && (
              ...
              {!isDashboard && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                  <motion.div 
                    ...
                    className="bg-gradient-to-r from-purple-500 to-violet-600 rounded-lg p-4 text-white shadow-lg"
                  >
                    <h3 className="text-lg font-semibold mb-1 opacity-90">Tổng người dùng</h3>
                    <div className="text-3xl font-bold">{totalUsers}</div>
                    <p className="text-sm mt-3 opacity-80">Tổng số người dùng đã đăng ký trong 12 tháng qua</p>
                  </motion.div>
                  ...
                  <motion.div 
                    ...
                    className="bg-gradient-to-r from-pink-500 to-rose-600 rounded-lg p-4 text-white shadow-lg"
                  >
                  ...
                  <motion.div 
                    ...
                    className="bg-gradient-to-r from-blue-500 to-sky-600 rounded-lg p-4 text-white shadow-lg"
                  >
                    <h3 className="text-lg font-semibold mb-1 opacity-90">Trung bình hàng tháng</h3>
                    <div className="text-3xl font-bold">
                      {(totalUsers / (analyticsData.length || 1)).toFixed(1)}
                    </div>
                    <p className="text-sm mt-3 opacity-80">Người dùng mới mỗi tháng</p>
                  </motion.div>
                </div>
              )}
            </div>
